...
export default {
  ...
  mounted(){
    if(this.$store.state.bookSelected){
      ...
      this.isbn = this.$store.state.bookSelected.isbn
      this.title = this.$store.state.bookSelected.title
      this.author = this.$store.state.bookSelected.author
      this.image_url = this.$store.state.bookSelected.image_url
      this.price = this.$store.state.bookSelected.price
    }
  },
  ...
  methods: {
    save(){
      if(this.isbn && this.title && this.image_url && this.author){
        if(this.price >= 0){
          const { isbn, title, image_url, price, author, id } = this
          const book = {
            isbn, title, image_url, price, author, id
          }
